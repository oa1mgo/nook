...
struct HookInstaller {
    ...
    static func installIfNeeded() {
        ...
        let pythonScript = hooksDir.appendingPathComponent("nook-state.py")
        ...
        if let bundled = Bundle.main.url(forResource: "nook-state", withExtension: "py") {
    ...
    private static func updateSettings(at settingsURL: URL) {
        ...
        for (event, config) in hookEvents {
            ...
            let cleanedEvent = existingEvent.compactMap { removingNookHooks(from: $0) }
    ...
    nonisolated private static func removingNookHooks(from entry: [String: Any]) -> [String: Any]? {
    ...
    nonisolated private static func isNookHook(_ hook: [String: Any]) -> Bool {
        let cmd = hook["command"] as? String ?? ""
        return cmd.contains("nook-state.py")
    }
}
